...
"template_output" フィールドには、必ず下記テンプレートを**そのまま**使い、〇や空欄を実際の値・文章で埋めた文字列を入れてください。
改行は \\n で表現し、箇条書きの「•」や「・」の記号も必ず含めてください。
...
出力するJSONの構造：
{
  ...
  "template_output": "<上記テンプレートを実際の値で埋めた文字列。改行は\\nで表現>"
}`;

  if (!sources || sources.length === 0) return base;

  // ソース内に「出力テンプレート指示」が書かれている場合は参考資料から除外し、
  // システムプロンプト側のテンプレート定義（template_output フィールド）に一本化する。
  // テンプレート指示ソースかどうかは「以下のテンプレート」「template_output」等のキーワードで判定。
  ...
  const isTemplateSource = (s) =>
    TEMPLATE_KEYWORDS.some(kw => s.content.includes(kw) || s.title.includes(kw));

  const refSources = sources.filter(s => !isTemplateSource(s));

  if (refSources.length === 0) return base;
