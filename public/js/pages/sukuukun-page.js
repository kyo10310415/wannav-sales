...
営業: ぜひ○○さんのことをもっと教えていただければと思いまして…"
  ...
  _updateSourceBadge() {
    ...
    if (evalBadge) {
      if (this.sources.length === 0) {
        ...
        const tplSources = this.sources.filter(s => this._isTemplateSource(s));
        const refSources = this.sources.filter(s => !this._isTemplateSource(s));
        const tplPart = tplSources.length > 0
          ? `<span style="background:#ede9fe;color:#7c3aed;font-size:10px;padding:1px 6px;border-radius:8px;font-weight:700">📋 テンプレート ${tplSources.length}件</span> `
          : `<span style="background:#fef3c7;color:#92400e;font-size:10px;padding:1px 6px;border-radius:8px;font-weight:700">⚠️ テンプレート未登録（デフォルト使用中）</span> `;
        const refPart = refSources.length > 0
          ? `<span style="color:#059669"><i class="fas fa-check-circle"></i> 参照ソース ${refSources.length}件</span>`
          : '';
        evalBadge.innerHTML = `<span style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">${tplPart}${refPart}</span>`;
      }
    }
  },

  // ソースがテンプレートソースかどうかを判定（バックエンドと同じロジック）
  _isTemplateSource(s) {
  ...
  _renderSourceList() {
    ...
    // テンプレートソースを先頭に、その後に参考資料ソースを表示
    const sorted = [
      ...this.sources.filter(s => this._isTemplateSource(s)),
      ...this.sources.filter(s => !this._isTemplateSource(s)),
    ];

    wrap.innerHTML = sorted.map(s => {
      const isTpl = this._isTemplateSource(s);
      const borderColor = isTpl ? '#a78bfa' : 'transparent';
      const hoverBg    = isTpl ? '#f5f3ff' : '#fffbeb';
      const hoverBorder= isTpl ? '#7c3aed' : '#fde68a';
      const iconColor  = s.source_type==='pdf' ? '#dc2626' : (isTpl ? '#7c3aed' : '#6b7280');
      const iconClass  = s.source_type==='pdf' ? 'fa-file-pdf' : (isTpl ? 'fa-file-alt' : 'fa-align-left');
      const tplBadge   = isTpl
        ? `<span style="font-size:9px;background:#ede9fe;color:#7c3aed;border-radius:8px;padding:1px 6px;font-weight:700;white-space:nowrap">📋 テンプレート</span>`
        : '';
      return `
      <div class="source-item" id="source-item-${s.id}"
        style="padding:8px 10px;border-radius:7px;margin-bottom:4px;cursor:pointer;border:1px solid ${borderColor};transition:all 0.15s;display:flex;align-items:flex-start;gap:8px;${isTpl?'background:#faf5ff;':''}"
        onmouseenter="this.style.background='${hoverBg}';this.style.borderColor='${hoverBorder}'"
        onmouseleave="this.style.background='${isTpl?'#faf5ff':'transparent'}';this.style.borderColor='${borderColor}'"
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:10px;color:#9ca3af;margin-top:2px;display:flex;align-items:center;gap:5px;flex-wrap:wrap">
            ${tplBadge}
            ${s.char_count ? `<span>${s.char_count.toLocaleString()}文字</span>` : ''}
            ${s.file_name ? `<span>・${Utils.escHtml(s.file_name)}</span>` : ''}
          </div>
        </div>
      ...
      <div style="width:100%">
        ...
        <div style="margin-bottom:14px;border-radius:10px;border:2px solid #f59e0b;overflow:hidden">
          <div style="background:#f59e0b;padding:8px 14px;display:flex;align-items:center;justify-content:space-between">
            ...
            <button onclick="SukuukunPage._copyTemplateOutput(this)"
        ...
        <!-- 詳細（アコーディオン：template_output がある場合は折りたたみ） -->
        ${data.template_output ? `
        <div style="margin-bottom:12px;border-radius:8px;border:1px solid #e5e7eb;overflow:hidden">
          <button onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'block':'none';this.querySelector('.acc-arrow').style.transform=this.nextElementSibling.style.display==='none'?'rotate(0deg)':'rotate(180deg)';"
            ...
            <i class="fas fa-chevron-down acc-arrow" style="font-size:10px;color:#9ca3af;transition:transform 0.2s"></i>
          </button>
      ...
      body.innerHTML = `
        ...
        <div style="margin-bottom:14px;border-radius:10px;border:2px solid #f59e0b;overflow:hidden">
          <div style="background:#f59e0b;padding:8px 14px;display:flex;align-items:center;justify-content:space-between">
            ...
            <button onclick="SukuukunPage._copyTemplateOutput(this)"
              data-text="${Utils.escHtml(ev.template_output)}"
              style="background:rgba(255,255,255,0.25);border:none;border-radius:5px;color:white;font-size:10px;padding:3px 8px;cursor:pointer;font-weight:600">
              <i class="fas fa-copy"></i> コピー
            </button>
          </div>
          <div style="padding:14px 16px;background:#fffbeb;font-size:12px;line-height:1.85;color:#1f2937;white-space:pre-wrap;font-family:inherit">${Utils.escHtml(ev.template_output)}</div>
        </div>` : ''}
        ...
        <div style="margin-bottom:12px;border-radius:8px;border:1px solid #e5e7eb;overflow:hidden">
          ...
          <div style="display:none;padding:12px">
            ${ev.summary ? `<div style="margin-bottom:12px;padding:10px 12px;background:#eff6ff;border-radius:8px;border-left:3px solid #3b82f6">
              <div style="font-size:11px;font-weight:600;color:#1e40af;margin-bottom:4px"><i class="fas fa-comment-dots"></i> 総合コメント</div>
        ...
        <!-- template_output なし：そのまま展開表示 -->
        ${ev.summary ? `<div style="margin-bottom:14px;padding:10px 12px;background:#eff6ff;border-radius:8px;border-left:3px solid #3b82f6">
          <div style="font-size:11px;font-weight:600;color:#1e40af;margin-bottom:4px"><i class="fas fa-comment-dots"></i> 総合コメント</div>
          <div style="font-size:12px;color:#1f2937;line-height:1.65">${Utils.escHtml(ev.summary)}</div>
        </div>` : ''}
        <div style="font-size:12px;font-weight:600;color:#374151;margin-bottom:8px"><i class="fas fa-list-check" style="color:#f59e0b;margin-right:4px"></i>各項目の採点</div>
