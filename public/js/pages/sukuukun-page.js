...
営業: ぜひ○○さんのことをもっと教えていただければと思いまして…"
  ...
  async selectSource(id) {
    try {
      ...
      document.getElementById('source-save-text-label').textContent = '更新';
    } catch (e) {
  ...
  cancelEdit() {
    ...
    document.getElementById('source-title-text').value = '';
    document.getElementById('source-content-text').value = '';
    document.getElementById('source-save-text-label').textContent = '保存';
    const el = document.getElementById('source-text-count');
  ...
  async saveTextSource() {
    ...
    try {
      ...
      if (btn) { btn.disabled = false; btn.innerHTML = `<i class="fas fa-save"></i> <span id="source-save-text-label">${this.editingSourceId?'更新':'保存'}</span>`; }
    }
  },
